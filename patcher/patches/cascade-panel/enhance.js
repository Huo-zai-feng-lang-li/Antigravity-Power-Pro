...
// 默认系统提示词 - 极简版，防止LLM误解
const DEFAULT_SYSTEM_PROMPT = `你是一个提示词重写器。

任务：将输入的文本重写为更好的AI提示词。

规则：
1. 只输出重写后的提示词，不要任何其他内容
2. 不要回答问题，只重写提示词
3. 保持原始语言（中文/英文）
4. 让提示词更清晰、更具体
...
输入: 当前是什么版本
输出: 请告诉我当前项目/软件的版本号是多少？如果有多个版本文件，请列出所有相关的版本信息。

输入: 帮我改bug
输出: 请帮我分析并修复代码中的bug。请找出问题根源，提供修复方案，并解释修复原理。

注意：直接输出重写后的提示词，不要任何前缀或解释。`;
...
function formatContextForPrompt(context) {
  ...
  // 6. 最近的对话摘要 - 简化版，只提供主题而非详细内容
  // 注意：详细对话内容容易让LLM误解任务，所以只提供简要信息
  if (context.recentMessages && context.recentMessages.length > 0) {
    // 只取第一条消息的前30个字符作为主题提示
    const topic = context.recentMessages[0]?.content?.substring(0, 30) || '';
    if (topic) {
      parts.push(`[对话主题: ${topic}...]`);
    }
  }
