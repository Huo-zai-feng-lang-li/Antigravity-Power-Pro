...
| 修复项 | 根因 | 影响范围 |
|--------|------|----------|
| **防患于未然：Manager 侧边栏争抢焦点** | 用户提出极其敏锐的架构假设：如果 Manager 独立窗口内部也开启了左右侧边栏（例如文件树）。之前的扫描器仅按 `scrollHeight` 的大小排序。如果项目的文件树列表极长，就会导致主聊天区的按钮因为“高度比不过文件树”而去错误追踪文件树的滚动状态！本次修复给真实对话区赋予了 `+10000` 的王权级优先级，确保无论周边长列表多高，滚动按钮永远死锁在真正的对话框上。 | `manager-panel/scroll-to-bottom.js` |
